...
//---------------------------------------------------------------------
	...
	changeServerRoom = GTK_WIDGET(gtk_builder_get_object(builder, "changeServerRoom"));
	...
	entryServer = GTK_ENTRY(gtk_builder_get_object(builder, "entryServer"));
	...
	entryServerDetalii = GTK_ENTRY(gtk_builder_get_object(builder, "entryServerDetalii"));
	...
	snprintf(sesiune, sizeof(sesiune), "SES_NR_%d_", nrSesiune-1);
	 ...
	 //extrag data si ora actuala si o stochez
	   time_t t = time(NULL);
    	...
    	char command17[50], command3[100],command4[100],finalCommand[100];
    	...
    	while ((read = getline(&line2, &len, fp)) != -1) { //iau toate incaperile adaugate in incaperi.txt 
			printf("%s", line2);
			line_without_enter = strtok(line2,"\n"); //iau doar numele incaperii, fara \n
			//intru in directorul sesiunii curente si formez directoarele pt fiecare server
			line_without_enter = strtok(sesiune,"\n"); //iau doar numele incaperii, fara \n
			snprintf(command3,sizeof(command3),"cd %s", sesiune);
			strcpy(finalCommand,command3);
			snprintf(command4,sizeof(command4),"mkdir %s", line2);
			strcat(finalCommand,";");
			strcat(finalCommand,command4);
			system(finalCommand);
			printf("\n\nFolder %s nume server %s\n\n",sesiune,line2);
...
void on_all_rooms_added_button_clicked(GtkButton *b3){
    ...
    char *line_without_enter;
    char text_label[100];
    char command[100],command3[100],command4[100];
    char commandFolder[100];
   ...
   while ((fgets(line2, 1024, nr_sesiunii)) != NULL)
	...
	//intru in directorul sesiunii curente si creez directoare noi cu numele fiecarui server, adica numele fiecarei incaperi
	sprintf(commandFolder,"cd %s; sudo mkdir %s", sesiune, line);
	system(commandFolder);
	
	line_without_enter = strtok(sesiune,"\n"); //iau doar numele incaperii, fara \n
	snprintf(command3,sizeof(command3),"cd %s", sesiune);
	strcpy(commandFolder,command3);
	snprintf(command4,sizeof(command4),"mkdir %s", line);
	strcat(commandFolder,";");
	strcat(commandFolder,command4);
	system(commandFolder);
	printf("\n\nFolder %s nume server %s\n\n",sesiune,line);
...
///butonul ce reia testul pentru aceeasi incapere
...
void on_changeServerRoom_clicked(GtkButton *b)
{

	char command[100], command2[100],command3[100],moveCommand[100];
	FILE *fp;
	char * line = NULL;
        size_t len = 0;
	char *line_without_enter;
	...
	char *nume_fisier;
	 ...
	 FILE *nr_sesiunii;
  	 ...
  	 char serverRoomName[100];
  	 sprintf(serverRoomName, "%s\n", gtk_entry_get_text(entryServer));	//iau numele incaperii in care este serverul 
   ...
   while ((fgets(line2, 1024, nr_sesiunii)) != NULL)
    ...
    snprintf(sesiune,sizeof(sesiune),"SES_NR_%d*/%s",nrSesiune,serverRoomName); //aici am calea spre folderul serverului din sesiunea curenta
	...
	system(command2);
	 ...
	 fp = fopen("incaperi.txt", "r");
  	 if (fp == NULL)
      	...
      	//creez un fisier gol cu numele incaperii in care se afla serverul	 
        char serverFileCom[100];
        char sn[100];
        strcpy(sn,serverRoomName);
        strtok(sn,"\n");
        strcat(sn,".txt");
    
        snprintf(serverFileCom,sizeof(serverFileCom),"sudo touch %s",sn);
        system(serverFileCom); 

	char room[100];
	char comRem[50];
	
    	while ((read = getline(&line, &len, fp)) != -1) 
    	{ //iau toate incaperile adaugate in toate_testele.txt 
		//printf("%s", line);
		...
		printf("\nFOLDER %s\n",serverRoomName);
			...
			snprintf(comRem,sizeof(comRem),"sudo rm %s",nume_fisier);
			system(comRem);

    }
    strcpy(moveCommand,"sudo cp -r rezultate_finale.txt ");
    strcat(moveCommand,sesiune);
    system(moveCommand);
    
    //sterg din folderul principal
    char removeRezCom[100];
    strcpy(removeRezCom, "sudo rm rezultate_finale.txt; sudo rm rezultate_partiale.txt");
    system(removeRezCom);
    
    char comIndice[300];
    strtok(sesiune,"\n");
    snprintf(comIndice,sizeof(comIndice),"sudo cd %s; sudo gcc indice_final.c -o indice_final ; sudo ./indice_final rezultate_finale.txt",sesiune);
					//genereaza rez_partiale.txt
    printf("\nCOMANDA INDICE %s\n",comIndice);
    system(comIndice);
...
//pt a muta ploturile in fiecare folder
void on_entryServer_changed(GtkEntry *e)
{
   FILE *nr_sesiunii;
   char * nr_ses;
   char line[100];
   ...
   while ((fgets(line, 1024, nr_sesiunii)) != NULL)
    ...
    char serverRoomName[100];
    //iau numele incaperii in care este serverul 
    sprintf(serverRoomName, "%s\n", gtk_entry_get_text(entryServer)); 
    strcat(sesiune,"/");
    strcat(sesiune,serverRoomName); //voi avea in sesiune calea spre folderul serverului selectat din sesiunea curenta
    
    strtok(sesiune,"\n");
    
    char movePlot[300];
    snprintf(movePlot, sizeof(movePlot), "cp rate.p %s; cp rata.p %s; cp duplicate.p %s; cp duplicates.p %s; cp power.p %s; cp putere.p %s; cp indice_final.c %s  ",sesiune,sesiune,sesiune,sesiune,sesiune,sesiune,sesiune);
    printf("\n\n COMANDA MOVE PLOT %s \n\n",movePlot);
   system(movePlot);
...
void on_rateButton_clicked(GtkButton *b){
  ...
  //iau numarul sesiunii curente din fisierul nr_sesiunii.txt
   ...
   while ((fgets(line, 1024, nr_sesiunii)) != NULL)
    ...
    char comanda[150],command12[150];
	...
	char serverRoomName[100];
  	sprintf(serverRoomName, "%s\n", gtk_entry_get_text(entryServerDetalii));	//iau numele incaperii in care este serverul 
	strcat(sesiune,"/");
	strcat(sesiune,serverRoomName); //voi avea in sesiune calea spre folderul serverului selectat din sesiunea curenta
	...
	snprintf(command, sizeof(command), "cd %s; cp %s data.txt",sesiune,aux_incapere);
	printf("\n COMANDA SCHIMBARE DENUMIRE %s\n",command);
	system(command);
	
	snprintf(command1, sizeof(command1), "cd %s;",sesiune);
	//execut scriptul ce face plotul
	gboolean T = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radioEN));
		if(T)
			strcat(command1, "setsid sudo gnuplot rate.p" );
		else
			strcat(command1, "setsid sudo gnuplot rata.p" );
	system(command1);
	
	
	 //modific numele imaginii cu numele_fisierului.png
        snprintf(command7,sizeof(command7),"cd %s; mv data.png %s",sesiune,png_name);
        system(command7);
    
	//afisez imaginea
	char cale_png[50];
	strtok(sesiune,"\n");
	snprintf(cale_png, sizeof(cale_png), "%s/",sesiune);
	strcat(cale_png,png_name);
	
	printf("\n\nCALE PNG %skkk \n\n",cale_png);
	
	char newCom[100];
	snprintf(newCom,sizeof(newCom),"mv %s data.png",cale_png);
	system(newCom);
...
void on_powerButton_clicked(GtkButton *b){
  ...
  //iau numarul sesiunii curente din fisierul nr_sesiunii.txt
   ...
   while ((fgets(line, 1024, nr_sesiunii)) != NULL)
	...
	gboolean T = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radioEN));
		if(T)
			strcat(command1, "setsid sudo gnuplot power.p" );
		else
			strcat(command1, "setsid sudo gnuplot putere.p" );
	system(command1);
...
void on_dupButton_clicked(GtkButton *b){
  ...
  //iau numarul sesiunii curente din fisierul nr_sesiunii.txt
   ...
   while ((fgets(line, 1024, nr_sesiunii)) != NULL)
	...
	gboolean T = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radioEN));
		if(T)
			strcat(command1, "setsid sudo gnuplot duplicates.p" );
		else
			strcat(command1, "setsid sudo gnuplot duplicate.p" );
	system(command1);
...
void on_resetButton_clicked(GtkButton *b)
{
	...
	char command[150];
	strcpy(command,"sudo rm reset.txt; sudo bash ./reset.sh");
	system(command);
